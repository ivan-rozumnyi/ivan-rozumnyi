<!doctype html>

<html>
<head>
    <meta charset="utf-8">
    <title>Web сервером і клієнтом No1</title>
</head>
<body bgcolor="#EEEEEE" text="black" link="blue">

<h1 align="center">Web сервером і клієнтом No1</h1>
<hr>

<!-- главная картинка -->
<p align="center">
    <img src="/ivan-rozumnyi/картинки-для-сайта/картинка1.png" width="200">
</p>


Цей текст являє дані, отримані в результаті роботи PHP:<br>

<?php
$todaysdate = date("d-m-Y");
echo $todaysdate;
?>

<br>
<p align="center">
    <a href="Перемикачів-та-прапорців-сторінка">Web сервером і клієнтом сторінку</a>
</p>

<p align="center">
    <em>Ця сторінка для лабораторної роботи 2.5</em>
</p>

</body>
</html>